...
from pygsm import GSMObserver, GSMObserver2016
...
class TskySim():
    ...
    def __init__(self, Tsky_file=None, beam_file=None, df=1.0, f_min=100.0, f_max=200.0, dlst=0.5,
                 pols=['E', 'N'], lat=-30.7224, lon=21.4278, elev=1100,gsm=2008):
        """
        Initialize class
        Args:
...
class auto_data():
    ...
    def __init__(self, data_dir='/data6/HERA/data/2458042/KM_uvR_files/', filestart='zen.*',
        ...
        if self.use_npz:
            data_file = np.load(self.data_dir+npz_file)
